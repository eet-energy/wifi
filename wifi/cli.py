...
from wifi.utils import match as fuzzy_match
from wifi.utils import print_table
...
def scan_command(args):
    print_table(
        [
            [cell.signal, cell.ssid, "protected" if cell.encrypted else "unprotected"]
            for cell in Cell.all(args.interface)
        ]
    )
...
def arg_parser():
    ...
    parser.add_argument(
        "-i", "--interface", default="wlan0", help="Specifies which interface to use (wlan0, eth0, etc.)"
    )
    parser.add_argument(
        "-f", "--file", default="/etc/network/interfaces", help="Specifies which file for scheme storage."
    )
    ...
    scheme_help = (
        "A memorable nickname for a wireless network."
        "  If SSID is not provided, the network will be guessed using SCHEME."
    )
    ssid_help = (
        "The SSID for the network to which you wish to connect."
    ...
    parser_connect.add_argument(
        "scheme", help="The nickname of the network to which you wish to connect.", metavar="SCHEME"
    )
    parser_connect.add_argument(
        "-a",
        "--ad-hoc",
    ...
    parser_autoconnect = subparsers.add_parser(
        "autoconnect",
        help="Searches for saved schemes that are currently" " available and connects to the first one it finds.",
    )
...
def main():
    ...
    try:
        if "WIFI_AUTOCOMPLETE" in os.environ:
            autocomplete(int(os.environ["COMP_CWORD"]), os.environ["COMP_WORDS"].split(), subparsers)
        else:
